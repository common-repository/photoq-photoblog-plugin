	...
	<div id="commonInfo<?php if(isset($_POST['ftp_upload'])) echo 'Ftp'; ?>">		
		...
		<?php 
			if ( function_exists('wp_nonce_field')){
				wp_nonce_field('photoq-editBatch', 'photoq-editBatch');
			}
		?>
		<div id="poststuff">
...
<?php if(!isset($_POST['ftp_upload'])): ?>
	<form action="edit.php?page=whoismanu-photoq.php" method="post" enctype="multipart/form-data">
		<?php 
			if ( function_exists('wp_nonce_field')){
				wp_nonce_field('photoq-ftpImport', 'photoq-ftpImport');
			}
		?>
		<div class="tablenav">
			...
			<?php else: ?>
				...
				<?php 
					if ( function_exists('wp_nonce_field')){
						wp_nonce_field('photoq-uploadBatch', 'photoq-uploadBatch');
					}
				?>
			<?php endif; ?>
			</div>
			...
			<?php if(!is_multisite() && $this->_oc->getValue('enableFtpUploads')): ?>
