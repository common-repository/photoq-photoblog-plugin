...
define('REUSABLEOPTIONS_PATH', dirname(dirname(__FILE__)).'/');
...
/**
 * The RO_OptionController:: class manages everything related to WordPress options:
 * Loading, Saving, Updating.
 *
 * @author  M.Flury
 * @package ReusableOptions
 */
class RO_OptionController
{
	 ...
	 * @var object RO_Visitor_RenderOptionVisitor
	 * @access private
	 */
	 ...
	 * @var object RO_Visitor_UpdateOptionVisitor
	 * @access private
	 */
	 ...
	 * 						WordPress Database.	
	 * @param object RO_Visitor_RenderOptionVisitor $renderVisitor		The visitor object 
	 * 														used to render options.
	 * @access public
	 */
	function __construct($name, RO_Visitor_RenderOptionVisitor $renderVisitor = null, 
		RO_Visitor_UpdateOptionVisitor $updateVisitor = null)
	{
		...
		if(is_null($renderVisitor))
			$this->_renderOptionVisitor = new RO_Visitor_RenderOptionVisitor();
		else
			$this->_renderOptionVisitor = $renderVisitor;
		
		if(is_null($updateVisitor))
			$this->_updateOptionVisitor = new RO_Visitor_UpdateOptionVisitor();
		else
	 ...
	 * @param object RO_Option_ReusableOption $option	The option to be added.
	 *
	 * @return boolean		True if option was added, False if not.
	 * @access public
	 */
	function registerOption(RO_Option_ReusableOption $option)
	{
	...
	{
		$key = null;
		if(is_object($option) && is_a($option, RO_Option_ReusableOption))
			$key = $option->getName();
	...
	function render($optionName)
	{
		if(!array_key_exists($optionName, $this->_options))
			echo "<strong>Error in RO_OptionController::render():</strong> 
	...
	function _store()
	{
		...
		foreach ($this->_options as $option){
			if(is_a($option, RO_Option_TextFieldSiteOption))
	 ...
	 *
	 * @param object RO_Visitor_RenderOptionVisitor $visitor	The visitor object responsible
	 * 											  	for rendering the options.
	 * @access public
	 */
	function setRenderOptionVisitor(RO_Visitor_RenderOptionVisitor $visitor)
	{
