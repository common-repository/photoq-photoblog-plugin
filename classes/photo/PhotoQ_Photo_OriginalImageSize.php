	...
	function createPhoto($oldOriginalPath){
		...
		if(!PhotoQHelper::createDir($this->_location->getYearMonthDirPath()))
			throw new PhotoQ_Error_Exception(sprintf(
				_c('Error when creating directory: %s| dirname', 'PhotoQ'), 
		...
		if (!$srcDest->destinationExists()) {
			if(!PhotoQHelper::moveFile($srcDest))
				throw new PhotoQ_Error_Exception(sprintf(
					_c('Unable to move %s, posting aborted.| imgname', 'PhotoQ'), $this->_imgName));
		}else{
			throw new PhotoQ_Error_Exception(sprintf(_c('Image %s already exists, posting aborted.| imgname', 'PhotoQ'), $this->_imgName));
		}
	}
